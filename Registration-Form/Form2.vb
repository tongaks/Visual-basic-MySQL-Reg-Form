...
Public Class Form2
    ...
    Private Sub UpdateRow(rowIndex As Integer, columnName As String, newValue As String)
        Dim sql As String = "UPDATE registration_form SET [" & columnName & "] = @newVal WHERE ID = @id"
    ...
    Private Sub LoadRegistrationDB()
        Dim sql As String = "SELECT * FROM registration_form ORDER BY ID ASC"
    ...
    Private Sub UpdateBtn_Click(sender As Object, e As EventArgs) Handles UpdateBtn.Click
        ...
        Dim birthDateFix As String = birthDate.ToString("M/d/yyyy")
        ...
        Dim message As String = $"ID: {id}{vbCrLf}" &
                        $"First Name: {firstName}{vbCrLf}" &
                        $"Middle Name: {middleName}{vbCrLf}" &
                        $"Surname: {surName}{vbCrLf}" &
                        $"Suffix: {suffix}{vbCrLf}" &
                        $"Sex: {sex}{vbCrLf}" &
                        $"Contact No.: {contactNo}{vbCrLf}" &
                        $"Email: {emailAddr}{vbCrLf}" &
                        $"Birth Date: {birthDateFix}{vbCrLf}" &
                        $"Address: {address}{vbCrLf}" &
                        $"Municipality: {municipality}{vbCrLf}" &
                        $"Postal Code: {postalCode}"

        MsgBox(message, MsgBoxStyle.Information, "Client Information")


        Dim sql As String = "UPDATE registration_form SET LastName = @lname, FirstName = @fname, MiddleName = @mname, Suffix = @suffix, Sex = @sex, ContactNo = @contactno, EmailAddress = @emailaddr, BirthDate = @dbirth, AddressIn = @addr, Municipality = @muni, PostalCode = @pcode WHERE ID = @id"
        Dim dbConnection As New MySqlConnection
        dbConnection = New MySqlConnection(ConnectionString)
        Dim cmd As MySqlCommand
        ...
        Try
            ...
            cmd.Parameters.AddWithValue("@dbirth", birthDateFix)
            ...
            Dim opt = MsgBox("Are you sure? ", vbYesNoCancel, "Warning")
            If opt = vbYes Then
                Dim res = cmd.ExecuteNonQuery()
                If (res > 0) Then
                    MsgBox("Successfully saved update", MsgBoxStyle.Information, "Success")
                    LoadRegistrationDB()
                Else
                    MsgBox("Error on updating the record", MsgBoxStyle.Critical, "Error")
                End If
            Else
                MsgBox("Operation cancelled", MsgBoxStyle.Critical, "Error")
            End If
    ...
    Private Sub DeleteBtn_Click(sender As Object, e As EventArgs) Handles DeleteBtn.Click
        ...
        Dim sql As String = "DELETE FROM registration_form WHERE ID = @id"
        ...
        Try
            ...
            If opt = vbYes Then
                Dim res = cmd.ExecuteNonQuery()
                If (res > 0) Then
                    MsgBox("Succesfully deleted", MsgBoxStyle.Information, "Deleted")
                    LoadRegistrationDB()
                Else
                    MsgBox("Error on deleting the record", MsgBoxStyle.Critical, "Error")
                End If
            Else
                MsgBox("Operation cancelled", MsgBoxStyle.Critical, "Error")
            End If
